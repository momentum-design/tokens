...
This is a public repository so anyone should be able to raise a PR with new tokens. Only internal temas can merge and publish new changes tho.
...
The main structure for the JSON file should be as following shown below. We'll discuss about `component-variation` later.
...
Each component is build from multiple parts. In our example, Chips have are built from 4 parts.
